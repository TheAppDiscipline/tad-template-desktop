...
1. Define the criteria in `.discipline/scorecard.yaml` (the template is the launch-vs-PROD scorecard-as-code material in the vault (sold separately), §4.1).
...
If it fails because scorecard.yaml is malformed: show the parser error and point to the 65a §4.1 schema in the vault (sold separately).
...
- `validate-scorecard.ts` does not exist: the template is not post-Wave 3.1. Suggest updating to the current version or installing the script manually from the implementation-status guide in the vault (sold separately).
