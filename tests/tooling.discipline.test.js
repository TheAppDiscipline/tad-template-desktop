...
// gate:strict is EXPECTED_FAIL on a pristine scaffold: check-desktop-release refuses the template's
// own Tauri identifier and a missing Cargo.lock, and it must keep refusing them. The risk with an
// "it always fails here" exemption is that nobody notices when it starts failing for a REAL reason,
// so this pins both halves: the scaffold fails, and a hydrated project passes.
test('check-desktop-release: the pristine scaffold fails, a hydrated project passes', () => {
  ...
  assert.match(getOutput(pristine), /Missing src-tauri\/Cargo\.lock/)
  ...
  fs.mkdirSync(path.join(hydrated, 'src-tauri'), { recursive: true })
  ...
  // No lockfile here on purpose: a hand-written one is refused by `cargo metadata --locked`, and
  // that half is proven in "a file named Cargo.lock is not a resolved dependency tree" below. What
  // this asserts is that hydrating the identifiers removes exactly those complaints and no others.
  const ready = run(hydrated)
  assert.notEqual(ready.status, 0, getOutput(ready))
  assert.doesNotMatch(getOutput(ready), /Replace Tauri identifier/)
  assert.doesNotMatch(getOutput(ready), /Replace Tauri productName/)
  assert.match(getOutput(ready), /Missing src-tauri\/Cargo\.lock/)
})
...
// The hydrated fixture used to write "# generated by cargo" into Cargo.lock and expect a PASS, so
// any file with that name bought a green. The resolver is now the judge: `cargo metadata --locked`
// reads the manifest and refuses to update anything, which a hand-written lockfile cannot survive.
test('check-desktop-release: a file named Cargo.lock is not a resolved dependency tree', () => {
  const hydrate = () => {
    ...
    fs.rmSync(path.join(dir, 'src-tauri', 'Cargo.lock'), { force: true })
    return dir
  }
  const run = (dir) => spawnSync(process.execPath, [path.join(repoRoot, 'tools', 'check_desktop_release_ready.js')], { cwd: dir, encoding: 'utf8' })

  // Everything hydrated EXCEPT the lockfile, which is a plausible-looking hand-written file.
  const faked = hydrate()
  fs.writeFileSync(path.join(faked, 'src-tauri', 'Cargo.lock'), '# generated by cargo\nversion = 3\n', 'utf8')
  const cargoAvailable = spawnSync('cargo --version', { encoding: 'utf8', shell: true }).status === 0
  const fake = run(faked)
  assert.notEqual(fake.status, 0, getOutput(fake))
  if (cargoAvailable) {
    assert.match(getOutput(fake), /not a resolved dependency tree/)
  } else {
    // Without the toolchain the check cannot verify the lockfile, and says so instead of passing.
    assert.match(getOutput(fake), /cargo is not available/)
  }

  // The other half only means anything where cargo can resolve: it needs the crates.io index.
  if (!cargoAvailable) return
  const real = hydrate()
  const generated = spawnSync('cargo generate-lockfile --manifest-path src-tauri/Cargo.toml', { cwd: real, encoding: 'utf8', shell: true })
  if (generated.status !== 0) return // offline or index unavailable: do not invent a verdict
  const ready = run(real)
