...
2. Look for a direct fix in the common gate errors guide in the vault (sold separately).
...
Write `.discipline/packets/SLICE_COMPLETION_PACKET.md` using the canonical "minimal SLICE_COMPLETION_PACKET structure" described in Step 5 - Implementation in The App Discipline vault (sold separately):
...
If `Deploy signal != not_ready`, also generate `.discipline/packets/DEPLOY_READINESS_PACKET.md` using the "minimal DEPLOY_READINESS_PACKET structure" described in Step 5 - Implementation in the vault (sold separately).
