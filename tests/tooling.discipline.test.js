...
function makeRunFixtureRepo(overrides = {}) {
  ...
  fs.writeFileSync(
    ...
    ['# discipline.md', '', '## 0) Profile', '- PROFILE: LITE', '- LANE: DESKTOP', '', '## Autonomy', `- level: ${level}`, '- builder: claude', '- validator: gemini', '- repair_max: 2', '', '## 1) Non-Negotiables', '- x', ''].join('\n'),
    'utf8',
  )
  fs.writeFileSync(
...
test('gate --changed: the buyer paths route to the gates this lane actually needs', () => {
  ...
  assert.ok(
    out.api.scripts.includes('check-desktop-release'),
    `an API change must run this lane's artifact check (check-desktop-release): ${out.api.scripts.join(', ')}`,
  )
  // And the artifact of a Tauri app is native code. `check-desktop-release` resolves dependencies
  // and reads the config; nothing in it compiles. Without this, an API change could close having
  // never built the Rust side, because the only command that does lived in gate:full alone.
  assert.ok(
    out.api.scripts.includes('check-native'),
    `an API change must compile the native side (check-native): ${out.api.scripts.join(', ')}`,
  )
...
// gate:strict is EXPECTED_FAIL on a pristine scaffold, and now for exactly ONE reason: the template's
// own Tauri identifier. The lockfile used to be the second reason, which meant the EXPECTED_FAIL
// covered two things at once and nobody could tell them apart; the template ships a resolved
// Cargo.lock now, so that half has to be a PASS. The risk with an "it always fails here" exemption is
// that nobody notices when it starts failing for a REAL reason, so this pins both halves: the
// scaffold fails on the placeholders alone, and hydrating them is all it takes to pass.
test('check-desktop-release: the pristine scaffold fails on its placeholders, and only those', () => {
  const run = (dir) => spawnSync(process.execPath, [path.join(repoRoot, 'tools', 'check_desktop_release_ready.js')], { cwd: dir, encoding: 'utf8' })

  // (a) The scaffold as shipped. This is the EXPECTED_FAIL in the phase's gate matrix.
  const pristine = run(repoRoot)
  assert.notEqual(pristine.status, 0, getOutput(pristine))
  assert.match(getOutput(pristine), /Replace Tauri identifier/)
  // The lockfile is NOT part of it any more: the template resolves its native dependencies.
  assert.doesNotMatch(getOutput(pristine), /Missing src-tauri\/Cargo\.lock/)
  assert.doesNotMatch(getOutput(pristine), /not a resolved dependency tree/)
  assert.ok(fs.existsSync(path.join(repoRoot, 'src-tauri', 'Cargo.lock')), 'the template ships a lockfile')

  // (b) The same check against a project that did what the message asks. Nothing else changes:
  // the shipped lockfile comes along, and that is enough for a PASS.
  const hydrated = fs.mkdtempSync(path.join(os.tmpdir(), 'desktop-hydrated-'))
  fs.cpSync(path.join(repoRoot, 'src-tauri'), path.join(hydrated, 'src-tauri'), { recursive: true })
  fs.rmSync(path.join(hydrated, 'src-tauri', 'target'), { recursive: true, force: true })
  const conf = JSON.parse(fs.readFileSync(path.join(repoRoot, 'src-tauri', 'tauri.conf.json'), 'utf8').replace(/^\uFEFF/, ''))
  conf.identifier = 'com.mycompany.myapp'
  conf.productName = 'My App'
  if (conf.app?.windows?.[0]) conf.app.windows[0].title = 'My App'
  fs.writeFileSync(path.join(hydrated, 'src-tauri', 'tauri.conf.json'), JSON.stringify(conf, null, 2), 'utf8')

  const ready = run(hydrated)
  const cargoAvailable = spawnSync('cargo --version', { encoding: 'utf8', shell: true }).status === 0
  if (cargoAvailable) {
    assert.equal(ready.status, 0, getOutput(ready))
    assert.match(getOutput(ready), /\[PASS\]/)
  } else {
    // Without the toolchain the lockfile cannot be verified, and the check says so rather than
    // passing. That is a different message from every placeholder complaint, which is the point.
    assert.match(getOutput(ready), /cargo is not available/)
    assert.doesNotMatch(getOutput(ready), /Replace Tauri identifier/)
  }
})
...
// The lockfile the template ships is a real one, so these are the three ways it can stop being real
// in a buyer's project: deleted, hand-written, or left behind after Cargo.toml moved on. All three
// are checked against temporary fixtures, never against the repo, so the shipped lockfile stays put.
test('check-desktop-release: a file named Cargo.lock is not a resolved dependency tree', () => {
  const cargoAvailable = spawnSync('cargo --version', { encoding: 'utf8', shell: true }).status === 0
  const run = (dir) => spawnSync(process.execPath, [path.join(repoRoot, 'tools', 'check_desktop_release_ready.js')], { cwd: dir, encoding: 'utf8' })

  // A copy of the shipped crate with the identifiers hydrated, so the ONLY thing left to complain
  // about is the lockfile. The shipped Cargo.lock comes along unless a case replaces it.
  const fixture = (mutate) => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'desktop-lock-'))
    fs.cpSync(path.join(repoRoot, 'src-tauri'), path.join(dir, 'src-tauri'), { recursive: true })
    fs.rmSync(path.join(dir, 'src-tauri', 'target'), { recursive: true, force: true })
    const conf = JSON.parse(fs.readFileSync(path.join(repoRoot, 'src-tauri', 'tauri.conf.json'), 'utf8').replace(/^\uFEFF/, ''))
    conf.identifier = 'com.mycompany.myapp'
    conf.productName = 'My App'
    if (conf.app?.windows?.[0]) conf.app.windows[0].title = 'My App'
    fs.writeFileSync(path.join(dir, 'src-tauri', 'tauri.conf.json'), JSON.stringify(conf, null, 2), 'utf8')
    if (mutate) mutate(path.join(dir, 'src-tauri'))
    return dir
  }

  // (a) Deleted. The message names the file and what produces it.
  const missing = run(fixture((crate) => fs.rmSync(path.join(crate, 'Cargo.lock'), { force: true })))
  assert.notEqual(missing.status, 0, getOutput(missing))
  assert.match(getOutput(missing), /Missing src-tauri\/Cargo\.lock/)

  // (b) Hand-written. It has the right name and says the right words and resolves nothing.
  const faked = run(fixture((crate) => fs.writeFileSync(path.join(crate, 'Cargo.lock'), '# generated by cargo\nversion = 4\n', 'utf8')))
  assert.notEqual(faked.status, 0, getOutput(faked))

  // (c) Real, but stale: Cargo.toml moved on and nobody regenerated it. Bumping the crate's own
  // version is enough, and needs no network, because the lockfile records that version too.
  const stale = run(fixture((crate) => {
    const manifest = path.join(crate, 'Cargo.toml')
    const bumped = fs.readFileSync(manifest, 'utf8').replace(/^version\s*=\s*"[^"]+"/m, 'version = "9.9.9"')
    fs.writeFileSync(manifest, bumped, 'utf8')
  }))
  assert.notEqual(stale.status, 0, getOutput(stale))

  if (cargoAvailable) {
    // The resolver is what says so, in the words it uses.
    assert.match(getOutput(faked), /not a resolved dependency tree/)
    assert.match(getOutput(stale), /not a resolved dependency tree/)
  } else {
    // Without the toolchain the check cannot verify the lockfile, and says so instead of passing.
    assert.match(getOutput(faked), /cargo is not available/)
    assert.match(getOutput(stale), /cargo is not available/)
  }

  // (d) The lockfile the template ships, untouched. Only meaningful where cargo can resolve.
  if (!cargoAvailable) return
  const ready = run(fixture(null))
  assert.equal(ready.status, 0, getOutput(ready))
  assert.match(getOutput(ready), /\[PASS\]/)
})
